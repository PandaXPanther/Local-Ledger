        ...
        <tbody className="divide-y divide-border bg-surface">
          {rows.map(row => (
            <tr key={`${row.rank}-${row.href}`} className="table-row">
              <td className="px-4 py-3 font-mono tabular-nums font-bold text-text-muted">{row.rank}</td>
              <td className="px-4 py-3 font-semibold"><Link href={row.href} className="editorial-link text-accent">{row.name}</Link></td>
              <td className="px-4 py-3 text-right font-mono tabular-nums font-bold text-ink">{row.value}</td>
